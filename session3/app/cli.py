...
from app.services.meeting_service import create_meeting, list_meetings as service_list_meetings
...
def list_meetings_cli():
    ...
    meetings_list = service_list_meetings()
    ...
    print(f"Total: {len(meetings_list)} meetings")


def create_meeting_cli(title: str, date: str, owner: str):
    """Create a new meeting using the service"""
    new_meeting = create_meeting(title=title, date=date, owner=owner)
...
def main():
    ...
    # create-meeting command (refactored to not require ID)
    ...
    create_parser.add_argument("--date", required=True, help="Meeting date (YYYY-MM-DD)")
    create_parser.add_argument("--owner", required=True, help="Meeting owner")
    ...
    if args.command == "list-meetings":
        ...
        create_meeting_cli(args.title, args.date, args.owner)
